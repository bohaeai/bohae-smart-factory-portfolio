...
<td align="center" width="25%">
<h3>MES / OPS</h3>
<sub>통합 운영 플랫폼<br/>운영·키오스크·LIMS·EAM·3D 트윈</sub>
</td>
<td align="center" width="25%">
<h3>CP-SAT</h3>
<sub>APS 스케줄링 엔진<br/>OR-Tools 기반 6 목적함수</sub>
</td>
<td align="center" width="25%">
<h3>ESP32 · C++</h3>
<sub>자체 펌웨어<br/>PCNT 기반 신호 손실 없는 수집</sub>
</td>
</tr>
</table>
...
### 3. MES/OPS 플랫폼 — *"통합 운영 스택"*

| 영역 | 구성 |
|------|------|
| **운영 콘솔** (OPS) | 생산계획 승인, 실행, 실적 관리, 의사결정 로그 |
| **키오스크/전광판** | 현장 터치 UI, 실시간 SSE, 자체 구축 |
...
📎 `api/app.py` — FastAPI 라우트 전체  
📎 `mes-frontend/` — 페이지, 컴포넌트, 피처, 스토어

### 4. 운영 자동화 — *"배포부터 감시까지"*

- **E2E 스모크**: 센서 → DB → API → 키오스크 → OPS → 실행 → 소크
- **소크 테스트** + 워치독 감시
- **Zero-downtime 배포** + onprem CLI (원커맨드 운영)
...
| 역할 | 상세 |
|------|------|
| **예산 수립** | 사업계획서 직접 작성, 분할 발주 |
| **임원 보고** | 발표자료 작성 + 임원 대상 사업·기술 브리핑 |
| **벤더 협업** | P&ID 센서 선정 |
| **유관 부서** | 전기팀(배선도), 전산팀(아키텍처/보안) 맞춤 브리핑 |
| **현장 실사** | 체크리스트 기반 실사, KSMS 수준진단, Zone·센서 포인트 매핑 |
| **AI 운용** | 에이전트(Codex/Claude/Gemini/ChatGPT) 역할 분담 병렬 운용 |
...
Phase 1 파일럿을 완료한 뒤, **15개 도메인 프로젝트**로 범위를 확장해 운영 중입니다.
...
| 2022.11 ~ 2023.05 | 오스템임플란트 | 영업 | 성형외과 채널 개척 |
| 군 복무 | 공군 | 항공기 정비병 | 기체 정비 |
